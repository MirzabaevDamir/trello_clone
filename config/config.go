...
func Load() Config {
	...
	c.PostgresDatabase = cast.ToString(getOrReturnDefault("POSTGRES_DATABASE", "trello"))
	c.PostgresUser = cast.ToString(getOrReturnDefault("POSTGRES_USER", "postgres"))
	c.PostgresPassword = cast.ToString(getOrReturnDefault("POSTGRES_PASSWORD", "samandar777"))
	c.PostgresConnectionTimeOut = cast.ToInt(getOrReturnDefault("POSTGRES_CONNECTION_TIMEOUT", 7))
	c.PostgresConnectionTry = cast.ToInt(getOrReturnDefault("POSTGRES_CONNECTION_TRY", 10))

	c.AdminUsername = cast.ToString(getOrReturnDefault("ADMIN_USERNAME", "damir"))
	c.AdminPassword = cast.ToString(getOrReturnDefault("ADMIN_PASSWORD", "damir"))
